...
RSpec.describe Product, type: :model do
  ...
  describe '.filter_by_title' do
    before(:each) do
      ...
      @product2 =  FactoryBot.create(:product, title: 'Faster Laptop')
    ...
    context "when a 'TV' title pattern is sent" do
      ...
      it 'returns the products matching' do
        expect(Product.filter_by_title('TV').sort).to match_array([@product1,
                                                                   @product4])
      end
    end
  end
  ...
  describe '.above_or_equal_to_price' do
    ...
    it 'returns the products which are above or equal to the price' do
      expect(Product.above_or_equal_to_price(100).sort).to match_array([@product1,
                                                                        @product3])
    end
  end
  ...
  describe '.below_or_equal_to_price' do
    ...
    it 'returns the products which are above or equal to the price' do
      expect(Product.below_or_equal_to_price(99).sort).to match_array([@product2,
                                                                       @product4])
    end
  end
  ...
  describe '.recent' do
    ...
    it 'returns the most updated records' do
      expect(Product.recent).to match_array([@product3, @product2, @product4,
                                             @product1])
    end
  end
  ...
  describe '.search' do
    ...
    context 'when an empty hash is sent' do
      it 'returns all the products' do
        expect(Product.search({})).to match_array([@product1, @product2, @product3,
                                                   @product4])
      end
    end
